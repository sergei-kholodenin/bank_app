...
/**
 * Represetns the RQuery class for working with DOM elements.
 */
class RQuery {
	/**
	 ...
	 * @param {HTMLElement} newElement - The new element to insert before selected element.
	 * @returns {RQuery} The current RQuery instance for chaining.
	 */
	before(newElement) {
	 ...
	 * @returns {RQuery|string} The current RQuery instance for chaining fo setting HTML content, or the current inner HTML when getting.
	 */
	html(HTMLcontent) {
	 ...
	 * @param {string} [textContent] - Optional HTML content to set. If not provided, the current text content will be returned.
	 * @returns {RQuery|string} The current RQuery instance for chaining fo setting HTML content, or the current text content when getting.
	 */
	text(textContent) {
	 ...
	 * @returns {string|RQuery} - If new value is provided, returns the RQuery instance. Otherwise, returns the current value of the input element.
	 */
	value(newValue) {
	...
	/**
	 * Get attributes and event listeners for an input element.
	...
	numberInput(limit) {
		...
		this.element.addEventListener('input', event => {
			...
			if (limit) {
				value = event.target.substring(0, limit)
	...
	/**
	 * Show the selected element by removing 'display' style property.
	 * @returns {RQuery} The current RQuery instance for chaining.
	 */
	show() {
	...
	/**
	 * Adds class or list of classes to the current element.
	...
	/**
	 * Removes class or list of classes to the current element.
	 ...
	 * @param {string} [value] - The value to set for the attribute. If not provided, the current value of attribute will be returned.
	...
	/**
	 * Removes an attribute from current element.
